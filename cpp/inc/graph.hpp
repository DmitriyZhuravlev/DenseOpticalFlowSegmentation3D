...
Edge create_edge(const cv::Mat &flow, int width, int x, int y, int x1, int y1, const DiffFunction &diff);
std::vector<Edge> build_graph(const cv::Mat &img, int width, int height, const DiffFunction &diff, bool neighborhood_8 = false);
...
public:
    ...
    std::vector<SegmentData> get_best_segments();
